...
private data class EasingEntry(
    val name: String,
    val easing: Easing,
    val color: Color
)
...
fun EaseList() {
    ...
    // just changes form 0 to 1
    var targetValue by remember { mutableFloatStateOf(0f) }

    /*
    // needed just when displaying progress
    ...
    Column(
        ...
        Button(onClick = { targetValue = if (targetValue == 0f) 1f else 0f }) {
            Text(if (targetValue == 0f) "Race!" else "Reset")
        }
    }
}
